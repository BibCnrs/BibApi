...
`SELECT name
FROM domain
LEFT JOIN unit_domain ON (domain.id = unit_domain.domain_id)
...
`SELECT name
FROM domain
LEFT JOIN institute_domain ON (institute_domain.domain_id = domain.id)
LEFT JOIN unit_institute ON (unit_institute.institute_id = institute_domain.institute_id)
WHERE unit_institute.unit_id = unit.id
...
LEFT JOIN domain ON (domain.id = unit_domain.domain_id) OR domain.id = institute_domain.domain_id
`
        )
...
export const addAllDomains = (unit) => {
    if(!unit) {
        return null;
    }
    return {
        ...
        all_domains: _.uniq(
            unit.domains
            .concat(unit.institutes_domains)
        )
    };
};
...
export default (client) => {
    ...
    queries.selectOne = function* selectOne(...args) {
        const unit = yield baseSelectOne(...args);

        return addAllDomains(unit);
    };

    queries.selectPage = function* selectPage(...args) {
        const units = yield baseSelectPage(...args);
        return units.map(addAllDomains);
    };
