...
const InistAccount = (client) => {
    ...
    const selectEzTicketInfoForId = function* selectEzTicketInfoForId(...args) {
        const [ezTicketInfo] = yield inistAccountClient.selectEzTicketInfoForId(...args);
    ...
    const insertOne = function* insertOne(inistAccount) {
        try {
            yield client.begin();
            const insertedInistAccount = yield inistAccountClient.insertOne(inistAccount);

            const communities = yield updateCommunities(inistAccount.communities, insertedInistAccount.id);
            const institutes = yield updateInstitutes(inistAccount.institutes, insertedInistAccount.id);
            const units = yield updateUnits(inistAccount.units, insertedInistAccount.id);
    ...
    const updateOne = function* (selector, inistAccount) {
        try {
            ...
            try {
                updatedInistAccount = yield inistAccountClient.updateOne(selector, inistAccount);
            } catch (error) {
                if(error.message !== 'no valid column to set') {
                    throw error;
                }
                updatedInistAccount = yield inistAccountClient.selectOne({ id: selector });
            }
            const communities = yield updateCommunities(inistAccount.communities, updatedInistAccount.id);
            const institutes = yield updateInstitutes(inistAccount.institutes, updatedInistAccount.id);
            const units = yield updateUnits(inistAccount.units, updatedInistAccount.id);
    ...
    const authenticate = function* authenticate(username, password) {
        const foundInistAccount = yield inistAccountClient.selectOneByUsername(username);
