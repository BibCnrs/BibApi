...
function InistAccount(client) {
    ...
    const selectAllForExport = function*() {
        const dataForExport = yield client.query({
            ...
            institute.code AS main_institute,
            ...
            JOIN inist_account_community ON (inist_account.id = inist_account_community.inist_account_id)
            JOIN community ON (inist_account_community.community_id = community.id)`,
            parameters: {},
        });
        const listForExport = [];
