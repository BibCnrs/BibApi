...
export const getInistAccount = function* () {
    const inistAccounts = yield prisma.inist_account.groupBy({
        by: ['id'],
    });
...
export const insertOne = function* (inistAccount) {
    const { units: inistAccountUnits, ... data } = inistAccount;
...
export const authenticate = function* (username, password) {
    const foundInistAccount = yield prisma.inist_account.findFirst({
        where: {
            username,
            password,
        },
