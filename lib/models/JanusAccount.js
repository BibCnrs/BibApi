...
const JanusAccount = function JanusAccount (client) {
    ...
    const selectEzTicketInfoForId = function* selectEzTicketInfoForId(...args) {
        ...
        return {
            username: `${result.mail}_O_${result.cnrs ? 'CNRS' : 'OTHER'}_I_${result.institute}_OU_${result.unit}`,
            groups: [
                ..._.uniq(
    ...
    const insertOne = function* insertOne(janusAccount) {
        try {
            ...
            const communities = yield updateCommunities(janusAccount.communities, insertedUser.id);
            const additional_institutes = yield updateAdditionalInstitutes(janusAccount.additional_institutes, insertedUser.id);
            const additional_units = yield updateAdditionalUnits(janusAccount.additional_units, insertedUser.id);
    ...
    const updateOne = function* (selector, janusAccount) {
        try {
            ...
            let updatedUser;
            try {
                updatedUser = yield janusAccountClient.updateOne(selector, janusAccount);
            } catch (error) {
            ...
            const communities = yield updateCommunities(janusAccount.communities, updatedUser.id);
            const additional_institutes = yield updateAdditionalInstitutes(janusAccount.additional_institutes, updatedUser.id);
            const additional_units = yield updateAdditionalUnits(janusAccount.additional_units, updatedUser.id);
