...
import { selectDomains, selectGroups, selectPrimaryInstituteDomains, selectPrimaryInstituteGroups, selectPrimaryUnitDomains, selectPrimaryUnitGroups } from '../queries/janusAccountQueries';
...
export const selectOneByUid = function* (uid) {
    const janusAccount = yield prisma.$queryRaw(
        `SELECT *, ARRAY(${selectPrimaryInstituteDomains}) as primary_institute_domains,
        ...
        ARRAY(${selectGroups}) AS groups
         FROM janus_account WHERE uid = $1`,
        uid,
    );
