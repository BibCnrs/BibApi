...
WHERE janus_account_institute.janus_account_id = janus_account.id ORDER BY name`
);
...
WHERE janus_account_unit.janus_account_id = janus_account.id ORDER BY code`
);
...
WHERE janus_account_domain.janus_account_id = janus_account.id ORDER BY name`
);
...
export default (client) => {
    ...
    queries.batchInsert = function* batchInsert(janusAccounts) {
        try {
            yield client.begin();
            const preparedUsers = yield janusAccounts.map(janusAccount => co(function* () {
            ...
            const wantedDomains = _.uniq(_.flatten(janusAccounts.map(janusAccount => janusAccount.domains).filter(domains => !!domains)));
            ...
            if(wantedDomains.length !== 0) {
                ...
                const janusAccountDomainsToInsert = insertedUsers
                .map((janusAccount, index) => ({
                    ...janusAccount,
                    domains: janusAccounts[index].domains
                }))
                .reduce((result, janusAccount) => {
                    if (!janusAccount.domains || janusAccount.domains.length === 0) {
                        return result;
                    }

                    return [
                        ...result,
                        ...janusAccount.domains.map(name => ({ domain_id: domainsByName[name].id, janus_account_id: janusAccount.id }))
                    ];
                }, []);
                yield janusAccountDomainQueries.batchInsert(janusAccountDomainsToInsert);
            }
