...
const selectAdditionalInstitutesGroups = (
`SELECT domain.gate
FROM domain
JOIN institute_domain ON (domain.id = institute_domain.domain_id)
JOIN institute ON (institute_domain.institute_id = institute.id)
...
const selectAdditionalUnitsGroups = (
`SELECT domain.gate
FROM domain
JOIN unit_domain ON (domain.id = unit_domain.domain_id)
JOIN unit ON (unit_domain.unit_id = unit.id)
...
const selectPrimaryUnitInstitutesGroups = (
`SELECT domain.gate
FROM domain
JOIN institute_domain ON (domain.id = institute_domain.domain_id)
JOIN unit_institute ON (unit_institute.institute_id = institute_domain.institute_id)
...
const selectAdditionalUnitsInstitutesGroups = (
`SELECT domain.gate
FROM domain
JOIN institute_domain ON (domain.id = institute_domain.domain_id)
JOIN unit_institute ON (unit_institute.institute_id = institute_domain.institute_id)
...
const returnFields = [
    ...
    `ARRAY(${selectAdditionalInstitutesGroups}) as additional_institutes_groups`,
    'primary_unit',
    ...
    `ARRAY(${selectAdditionalUnitsGroups}) as additional_units_groups`,
    `ARRAY(${selectPrimaryUnitInstitutesDomains}) as primary_unit_institutes_domains`,
    `ARRAY(${selectPrimaryUnitInstitutesGroups}) as primary_unit_institutes_groups`,
    `ARRAY(${selectAdditionalUnitsInstitutesDomains}) as additional_units_institutes_domains`,
    `ARRAY(${selectAdditionalUnitsInstitutesGroups}) as additional_units_institutes_groups`,
...
const selectOneByUidQuery = selectOne('janus_account', ['uid'], returnFields);

const selectEzTicketGroupsForIdQuery = selectOne('janus_account', ['id'], [
    'cnrs',
...
export const addAllDomains = (janusAccount) => {
    return {
        ...
        all_groups: _.uniq(
            ...
            .concat(janusAccount.additional_institutes_groups)
            .concat(janusAccount.primary_unit_groups)
            .concat(janusAccount.primary_unit_institutes_groups)
            .concat(janusAccount.additional_units_groups)
            .concat(janusAccount.additional_units_institutes_groups)
    ...
    queries.selectEzTicketGroupsForIdQuery = function* selectEzTicketGroupsForIdQuery(...args) {
        ...
        return [
            ..._.uniq(
                ...
                .concat(result.additional_institutes_groups)
                .concat(result.primary_unit_groups)
                .concat(result.primary_unit_institutes_groups)
                .concat(result.additional_units_groups)
                .concat(result.additional_units_institutes_groups)
