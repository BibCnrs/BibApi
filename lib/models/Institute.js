...
const instituteQueries = crud('institute', ['code', 'name'], ['id'], ['*'], [
    (queries) => {
        queries.selectOne.returnFields([
            ...
            'code',
            'name',
            `ARRAY(SELECT name FROM domain JOIN institute_domain ON (domain.id = institute_domain.domain_id) WHERE institute_domain.institute_id = $id ORDER BY name) AS domains`
        ]);
...
const selectOneByCodeQuery = selectOne('institute', ['code'], ['id', 'code', 'name']);
...
export default (client) => {
    ...
    queries.updateDomains = function* updateDomains(domainNames = [], institute) {
        const nextDomains = domainNames.length ? (yield domainQueries.selectByNames(domainNames)) : [];
        const nextDomainsId = nextDomains.map(domain => domain.id);
        const prevDomains = (yield domainQueries.selectByInstituteId(institute.id));
