...
const selectByQuery = selectPageQuery('institute', ['name'], ['id', 'code', 'name']);
const selectByIdsQuery = selectByOrderedFieldValuesQuery('institute', 'id', ['id', 'code', 'name']);

const upsertOnePerCodeQuery = upsertOneQuery('institute', ['code'], ['name']);
const selectOneByCodeQuery = selectOneQuery('institute', ['code'], ['id', 'code', 'name', `ARRAY(${selectCommunitiesName}) AS communities`]);

export default (client) => {
    const queries = client.link(instituteQueries);
