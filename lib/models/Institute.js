...
const Institute = function Institute (client) {
    ...
    const selectByJanusAccountId = function* (userId) {
        return yield instituteClient.selectByJanusAccountId(null, null, { janus_account_id: userId }, 'index', 'ASC');
    };

    const selectByInistAccountId = function* (inistAccountId) {
        return yield instituteClient.selectByInistAccountId(null, null, { inist_account_id: inistAccountId }, 'index', 'ASC');
    };

    const selectByUnitId = function* (unitId) {
        return yield instituteClient.selectByUnitId(null, null, { unit_id: unitId }, 'index', 'ASC');
    };
    ...
    const insertOne = function* insertOne(institute) {
        try {
            ...
            const communities = yield updateCommunities(institute.communities, insertedInstitute.id);
    ...
    const updateOne = function* (selector, institute) {
        try {
            ...
            try {
                ...
                updatedInstitute = yield instituteClient.selectOne({ id: selector });
            }

            const communities = yield updateCommunities(institute.communities, updatedInstitute.id);
    ...
    const selectByCodes = function* (codes) {
        const institutes = yield instituteClient.selectBy(null, null, { code: codes });
