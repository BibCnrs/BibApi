...
function DatabaseCommunity(client) {
    ...
    const assignCommunityToDatabase = function* (communityIds, databaseId) {
        return yield DatabaseCommunityClient.batchUpsert(communityIds.map((communityId) => ({ community_id: communityId, database_id: databaseId })));
    };

    const unassignCommunityFromDatabase = function* (communityIds, databaseId) {
        return yield DatabaseCommunityClient.batchDelete(communityIds.map(communityId => ({ community_id: communityId, database_id: databaseId })));
    };
