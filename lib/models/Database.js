...
function Database(client) {
    ...
    const insertOne = function* insertOne(database) {
        try {
            ...
            const communities = yield updateCommunities(database.communities, insertedDatabase.id);
    ...
    const updateOne = function* (selector, inistAccount) {
        try {
            ...
            let updatedDatabase;
            try {
                updatedDatabase = yield databaseClient.updateOne(selector, inistAccount);
            } catch (error) {
                if(error.message !== 'no valid column to set') {
                    throw error;
                }
                updatedDatabase = yield databaseClient.selectOne({ id: selector });
            }
            const communities = yield updateCommunities(inistAccount.communities, updatedDatabase.id);
