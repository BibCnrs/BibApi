...
const Community = function Community (client) {
    ...
    const selectByJanusAccountId = function* (userId) {
        return yield communityClient.selectByJanusAccountId(null, null, { janus_account_id: userId }, 'index', 'ASC');
    };

    const selectByInistAccountId = function* (inistAccountId) {
        return yield communityClient.selectByInistAccountId(null, null, { inist_account_id: inistAccountId }, 'index', 'ASC');
    };

    const selectByInstituteId = function* (instituteId) {
        return yield communityClient.selectByInstituteId(null, null, { institute_id: instituteId }, 'index', 'ASC');
    };

    const selectByUnitId = function* (unitId) {
        return yield communityClient.selectByUnitId(null, null, { unit_id: unitId }, 'index', 'ASC');
    };
