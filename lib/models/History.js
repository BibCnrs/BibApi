...
export const selectAlertToExecute = function* ({ date, limit }) {
    return yield prisma.$queryRaw`SELECT * FROM history WHERE last_execution + frequence <= ${date}::date AND has_alert IS true AND active IS true LIMIT ${limit}`;
};
...
export const deleteAllHistoriesNotAlertForUser = function* ({ user_id }) {
    return yield prisma.$queryRaw`DELETE FROM history WHERE user_id = ${user_id} AND has_alert = false`;
};
