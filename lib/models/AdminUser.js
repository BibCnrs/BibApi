...
const adminUserQueries = crudQueries('admin_user', ['username', 'password', 'salt'], ['id'], ['id', 'username'], [queries => {
    queries.selectOne.returnFields(['id', 'username']);
    queries.selectPage.returnFields(['id', 'username']);
}]);

export default (client) => {
    const queries = client.link(adminUserQueries);
    const baseInsertOne = queries.insertOne;
    const baseBatchInsert = queries.batchInsert;
    const baseUpdateOne = queries.updateOne;
    const selectOneByUsernameQuery = client.link(selectOneQuery('admin_user', ['username'], ['*']));
