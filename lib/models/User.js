...
const userQueries = crud('bib_user', ['username', 'password', 'salt', 'primary_institute', 'primary_unit'], ['id'], [
    ...
    (queries) => {
        queries.selectOne.returnFields([
            ...
            'username',
            'primary_institute',
            'ARRAY(SELECT id FROM institute JOIN bib_user_institute ON (institute.id = bib_user_institute.institute_id) WHERE bib_user_institute.bib_user_id = $id ORDER BY name) AS additional_institutes',
            'primary_unit',
            'ARRAY(SELECT id FROM unit JOIN bib_user_unit ON (unit.id = bib_user_unit.unit_id) WHERE bib_user_unit.bib_user_id = $id ORDER BY name) AS additional_units',
