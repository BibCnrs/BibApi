...
WHERE bib_user_institute.bib_user_id = bib_user.id ORDER BY name`
);
...
WHERE bib_user_unit.bib_user_id = bib_user.id ORDER BY code`
);
...
WHERE bib_user_domain.bib_user_id = bib_user.id ORDER BY name`
);
...
JOIN institute ON (institute_domain.institute_id = institute.id)
JOIN bib_user_institute ON (institute.id = bib_user_institute.institute_id)
WHERE bib_user_institute.bib_user_id = bib_user.id`
);
...
JOIN unit ON (unit_domain.unit_id = unit.id)
JOIN bib_user_unit ON (unit.id = bib_user_unit.unit_id)
WHERE bib_user_unit.bib_user_id = bib_user.id`
);
...
WHERE unit_institute.unit_id = bib_user.primary_unit`
);
...
JOIN institute_domain ON (domain.id = institute_domain.domain_id)
JOIN unit_institute ON (unit_institute.institute_id = institute_domain.institute_id)
JOIN bib_user_unit ON (bib_user_unit.unit_id = unit_institute.unit_id)
WHERE bib_user_unit.bib_user_id = bib_user.id`
);
...
const userQueries = crud('bib_user', ['username', 'primary_institute', 'primary_unit'], ['id'], [
    'id',
    'username',
    'primary_institute',
    'primary_unit'
...
const selectOneByUsernameQuery = selectOne('bib_user', ['username'], returnFields);
