...
export default (client) => {
    ...
    queries.selectOneByUserName = selectOne('bib_user', ['username'], [
        'id',
        'username',
        'password',
        'salt',
        'institute',
        'unit',
        `ARRAY(SELECT name FROM domain JOIN bib_user_domain ON (domain.id = bib_user_domain.domain_id) JOIN bib_user ON (bib_user_domain.bib_user_id = bib_user.id) WHERE bib_user.username = $username ORDER BY name) AS domains`
    ])(client);
