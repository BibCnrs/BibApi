...
const InistAccountUnit = function inistAccountInstituteQueries(client) {
    ...
    const assignUnitToInistAccount = function* (unitIds, inistAccountId) {
        return yield inistAccountInstituteClient.batchUpsert(unitIds.map((unitId, index) => ({ unit_id: unitId, inist_account_id: inistAccountId, index })));
    };

    const unassignUnitFromInistAccount = function* (unitIds, inistAccountId) {
        return yield inistAccountInstituteClient.batchDelete(unitIds.map((unitId) => ({ unit_id: unitId, inist_account_id: inistAccountId })));
    };
