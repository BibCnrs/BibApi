...
export const login = function* login() {
    ...
    const instituteCode = renaterHeader.refscientificoffice && renaterHeader.refscientificoffice.replace(/->.*/, '');

    const institute = instituteMap[instituteCode];
    ...
    if(!user || !user.domains || !user.domains[0] || user.domains.length === 0) {
