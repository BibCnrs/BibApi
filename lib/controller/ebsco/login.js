...
export const renaterLogin = function* login() {
    ...
    if (code && name) {
        yield this.instituteQueries.upsertOnePerCode({ code, name });
    }
    const institute = yield this.instituteQueries.selectOneByCode({ code });

    if(renaterHeader.ou) {
        yield this.unitQueries.upsertOnePerName({ name: renaterHeader.ou });
    }
    const unit = renaterHeader.ou ? yield this.unitQueries.selectOneByName({ name: renaterHeader.ou }) : null;

    yield this.userQueries.upsertOnePerUsername({
        username: renaterHeader.remote_user,
        primary_institute: institute && institute.id,
        primary_unit: unit && unit.id
    });

    const user = yield this.userQueries.selectOneByUsername(renaterHeader.remote_user);

    const domains = _.uniq(user.domains
    .concat(user.primary_institute_domains)
...
export const login = function* login() {
    ...
    const user = yield this.userQueries.authenticate(username, password);
