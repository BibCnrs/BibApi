import config, { auth, bibadmin_host } from 'config';
...
<p>Le nouveau compte ${getLink(account)} ressenmble aux comptes suivants : </p>
<ul>
...
Le nouveau compte ${getLink(account)} ressenmble aux comptes suivants :
${similarAccounts.map(({ uid, id }) => `- ${uid} : ${getHref(id)}\n`)}
`;

const getSimilarAlertMail = (account, similarAccounts) => ({
    from: 'assistance-portail@inist.fr',
    to: 'assistance-portail@inist.fr l',
    subject: 'Nouveau uid similaire',
...
export const renaterLogin = function* login() {
    ...
    if (similarAccounts.length) {
        const mail = getSimilarAlertMail(user, similarAccounts);
        yield sendMail(mail);
    }
