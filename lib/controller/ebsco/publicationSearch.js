...
import parsePublicationSearchQuery from '../../services/parsePublicationSearchQuery';

export const publicationSearch = function* publicationSearch(domainName) {
    const query = parsePublicationSearchQuery(this.query);
...
export const publicationSearchPure = function* publicationSearchPure(domainName) {
    ...
    const query = parsePublicationSearchQuery(this.query);
    try {
        this.body = yield ebscoPublicationSearch(query, sessionToken, authToken);
    } catch (error) {
