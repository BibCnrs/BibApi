...
export const getHistory = function* () {
    ...
    const historyEntries = yield this.historyQueries.selectPage(limit, offset, { user_id }, 'created_at', 'DESC');

    this.body = historyEntries.map(({ id, event, totalcount }) => ({ id, event, totalcount }));
};
