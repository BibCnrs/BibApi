...
app.use(function* () {
    ...
    const user = stateUser && stateUser.username ? yield this.userQueries.selectOneByUsername({ username: stateUser.username }) : this.user;
    ...
    const domains = yield this.domainQueries.selectByUserId(user.id);

    const url = generateEZTicket(this.query.gate, this.query.url, user.username, domains.map(d => d.gate));
    this.redirect(url);
});
