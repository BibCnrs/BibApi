...
app.use(function* () {
    ...
    if (!user || user.domains.indexOf(domain.name) === -1) {
        this.redirect(`http://${this.host}/ezticket/login?gate=${encodeURIComponent(this.query.gate)}&url=${encodeURIComponent(this.query.url)}`); // TODO redirect to janus when available
        return;
    }
