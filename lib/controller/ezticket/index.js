...
app.use(function* () {
    ...
    const inistAccount = stateUser && stateUser.username ? stateUser : this.user;
    if (!inistAccount) {
        this.redirect(`ezticket/login?gate=${encodeURIComponent(this.query.gate)}&url=${encodeURIComponent(this.query.url)}`);
        return;
    }

    if (inistAccount.all_domains.indexOf(domain.name) === -1) {
        this.status = 401;
        return;
    }

    const url = generateEZTicket(this.query.gate, this.query.url, inistAccount.username, inistAccount.all_groups);
    this.redirect(url);
});
