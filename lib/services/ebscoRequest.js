...
export function handleEbscoError(error) {
    ...
    let newError = new Error(error.error.ErrorDescription || error.error.Reason || JSON.stringify(error.error));

    newError.status = error.statusCode || 500;
    throw newError;
}

export default function* ebscoRequest(url, json, authToken, sessionToken) {
    const requestData = {
    ...
    const result = yield request.post(requestData)
    .catch(handleEbscoError)
    .catch(error => {
        const end = Date.now();
        ebscoLogger.info(`POST ${requestData.url} - ${error.statusCode} Failure`, { json, authToken, sessionToken, time: `took ${end - start}ms` });
        throw error;
    });
    const end = Date.now();

    ebscoLogger.info(`POST ${requestData.url} - 200 SUCCESS`, { json, authToken, sessionToken, time: `took ${end - start}ms` });
