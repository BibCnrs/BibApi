...
export const parseMetadoreSearch = (rawQuery) => {
    ...
    // TODO: choose the best way to build query for metadore API
    const query = `(${field}:"${term}")AND(attributes.types.resourceTypeGeneral:Dataset)`;

    // let query = term
    //     .split(' ')
    //     .map((term) => (field ? `(${field}:*${term}*)` : `(${term})`))
    //     .join('AND');
    // query += 'AND(attributes.types.resourceTypeGeneral:Dataset)';
