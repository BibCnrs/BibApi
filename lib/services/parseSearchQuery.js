...
export const parseMetadoreSearch = (rawQuery) => {
    const metadoreQuery = {
        query: JSON.parse(rawQuery.queries)[0].term,
        size: rawQuery.resultsPerPage,
    };
    if (rawQuery.currentPage && parseInt(rawQuery.currentPage) > 1) {
        // TODO update below code when metadore pagination is fixed
        metadoreQuery.page =
            (parseInt(rawQuery.currentPage) - 1) *
            parseInt(rawQuery.resultsPerPage);
    }
