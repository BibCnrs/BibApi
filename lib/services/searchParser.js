...
export const extractArticleLink = defaultToNull(function extractNoticeLink(result) {
    const directUrl = extractTargetFromItems('Name', 'URL', result.Items);
    if (directUrl) {
        return directUrl.replace(/&lt;link.*?&gt;/g, '').replace(/&lt;\/link&gt;/g, '');
    }

    const resolverLink = extractCustomLinks(result);
