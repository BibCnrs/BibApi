...
export const getUrlFromUnpaywall = function*(unpaywallUrl, domain) {
    try {
        ...
        const query = `{getDataUPW(dois:["${doi}"]){is_oa, best_oa_location{ url }}}`;
        const result = yield request({
            method: 'POST',
            url: 'https://unpaywall.inist.fr/graphql?sid=bibapi',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ query: query }),
        });
        const is_oa = JSON.parse(result).data.getDataUPW[0].is_oa;
        if (is_oa) {
            const url = JSON.parse(result).data.getDataUPW[0].best_oa_location
                .url;
