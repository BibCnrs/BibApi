...
export const parseFullTextHolding = function (fullTextHolding) {
    const coverage = fullTextHolding.CoverageDates ? fullTextHolding.CoverageDates
    .sort(({ end: a }, { end: b }) => parseInt(b) - parseInt(a))
    .map((coverageDate) => ({
...
export const extractFullTextHoldings = function(result) {
    ...
    return result.FullTextHoldings.map(parseFullTextHolding).sort((a, b) => {
        const endA = get(a, 'coverage[0].end.year', 0);
        const endB = get(b, 'coverage[0].end.year', 0);
