...
export const extractExportLinks = defaultToEmptyObject(
    function extractExportLinks(result) {
        return result.CustomLinks.filter(
            link => link.Category === 'other',
        ).reduce(function*(data, link) {
            if (link.Url.includes('api.unpaywall')) {
                link.Url = yield getUrlFromUnpaywall(link.Url, result.domain);
            }
            return {
                ...data,
...
export const getUrlFromUnpaywall = function*(unpaywallUrl, domain) {
    try {
        ...
        if (urlLocation) {
            ...
            return `${config.api_endpoint}/oa?sid=unpaywall&doi=${doi}&url=${url}&domaine=${domain}`;
        }
        return unpaywallUrl;
    } catch (error) {
        return unpaywallUrl;
...
export default function* articleParser(result, domain) {
    ...
    return {
        ...
        exportLinks: yield extractExportLinks(result),
