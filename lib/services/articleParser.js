...
export const isAvailable = defaultToFalse((result) => result.FullText.Text.Availability === '1');

export const hasLinkInNotice = defaultToFalse((result) => {
    if(isAvailable(result)) {
        return true;
    }

    return result.FullText.Links
    .filter(({ Type }) => Type === 'pdflink')
    .some(link => !link.Url);
});
...
export const extractArticleLinks = defaultTo({
    ...
    return {
        ...
        hasLinkInNotice: hasLinkInNotice(result) || false
    };
});
