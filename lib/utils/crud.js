...
export const getListParams = function getListParams(query) {
    const filter = query.filter ? JSON.parse(query.filter) : {};

    const page = query._page || 1;
    const perPage = query._perPage || 30;
...
export default function crud(model, idField = '_id', fields = [], options = {}, app = koa()) {
    ...
    fields = fields === '*' ? undefined : fields.join(' ');
    ...
    if (!onlyMethods.length || onlyMethods.indexOf('get') !== -1) {
        app.use(koaRoute.get('/', function* all() {
            ...
            findQuery.sort((sortDir.toLowerCase() === 'asc' ? '' : '-') + sortField)
            .skip(first)
            .limit(last - first + 1);
        ...
        app.use(koaRoute.get('/:id', function* one(id) {
            this.body = yield model.findOne({ [idField]: id }, fields);
        }));
    }
