...
const postgresCrud = (queriesFactory, availableMethods = ['GET', 'POST', 'PUT', 'DELETE']) => {
    ...
    app.use(koaRoute.get('/', function* () {
        ...
        if (queriesFactory.queries.filtersJoin) {
            const jointFields = Object.keys(filters)
            .filter(field => queriesFactory.queries.filtersJoin[field]);

            jointFields.forEach(field => queriesFactory.queries.selectPage.searchableFields([
