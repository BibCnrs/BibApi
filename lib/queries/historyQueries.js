...
const returnFields = ['id', 'user_id', 'created_at', 'event'];
...
const deleteEntriesCreatedBeforeThan = oldestDate => ({
    sql:
        'WITH deleted AS (DELETE FROM history WHERE created_at < $oldestDate::timestamp RETURNING *) SELECT count(*) FROM deleted;',
