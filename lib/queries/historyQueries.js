...
const selectAlertToExecute = ({ date, limit, offset }) => ({
    sql: `SELECT * FROM history WHERE last_execution + frequence <= $date::date AND has_alert IS true AND active IS true ORDER BY id ASC LIMIT $limit OFFSET $offset`,
    parameters: {
        date,
        limit,
        offset,
...
const enableOrDisableAllAlert = (activeValue, user_id) => ({
    sql:
        'UPDATE history SET active = $active WHERE has_alert = true AND user_id = $user_id',
    parameters: {
        active: activeValue,
        user_id,
    },
...
const countHistories = (user_id, has_alert) => ({
    sql:
        'SELECT COUNT(id) FROM history WHERE has_alert = $has_alert AND user_id = $user_id',
    parameters: {
        has_alert,
        user_id,
    },
