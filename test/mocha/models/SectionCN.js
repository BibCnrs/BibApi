...
describe('model SectionCN', function () {
    ...
    describe('selectOne', function () {
        ...
        before(function* () {
            primaryInstitute = yield fixtureLoader.createInstitute({ name: 'primary', code: '1' });
            secondaryInstitute = yield fixtureLoader.createInstitute({ name: 'secondary', code: '2' });
    ...
    describe('selectPage', function () {
        ...
        before(function* () {
            [ds50, ds51, ds52, ds53] = yield ['ds50', 'ds51', 'ds52', 'ds53']
    ...
    describe('updateOne', function () {
        ...
        beforeEach(function* () {
            [institute1, institute2, institute3] = yield ['institute1', 'institute2', 'institute3']
            .map(name => fixtureLoader.createInstitute({ name, code: name }));
        ...
        it('should throw an error if trying to add an institute to primary_institutes which does not exists and abort modification', function* () {
            let error;
            try {
                yield sectionCNQueries.updateOne(section.id, { primary_institutes: ['nemo', institute1.id] });
            } catch (e) {
                error = e.message;
            }
            ...
            assert.deepEqual(sectionCNInstitutes, [
                { section_cn_id: section.id, institute_id: institute1.id, index: 0 },
            ]);
        });
        ...
        it('should throw an error if trying to add an institute to secondary_institutes which does not exists and abort modification', function* () {
            let error;
            try {
                yield sectionCNQueries.updateOne(section.id, { secondary_institutes: ['nemo', institute1.id] });
            } catch (e) {
                error = e.message;
            }
            ...
            assert.deepEqual(sectionCNInstitutes, [
                { section_cn_id: section.id, institute_id: institute2.id, index: 0 },
            ]);
        });

        it('should replace primary_institute with given  institute', function* () {
            yield sectionCNQueries.updateOne(section.id, { primary_institutes: [institute2.id, institute3.id] });
            ...
            assert.deepEqual(sectionCNPrimaryInstitute, [
                { section_cn_id: section.id, institute_id: institute2.id, index: 0 },
                { section_cn_id: section.id, institute_id: institute3.id, index: 1 },
            ]);
        });

        it('should replace secondary_institute with given  institute', function* () {
            yield sectionCNQueries.updateOne(section.id, { secondary_institutes: [institute1.id, institute3.id] });
    ...
    describe('insertOne', function () {
        ...
        beforeEach(function* () {
            [primary, secondary] = yield ['primary', 'secondary']
            .map(name => fixtureLoader.createInstitute({ name, code: name }));
        });
        ...
        it('should add given institutes if they exists', function* () {
            ...
            assert.deepEqual(sectionCNPrimaryInstitutes, [
                { section_cn_id: section.id, institute_id: primary.id, index: 0 },
            ]);
            ...
            assert.deepEqual(sectionCNSecondaryInstitutes, [
                { section_cn_id: section.id, institute_id: secondary.id, index: 0 },
            ]);
        });
        ...
        it('should throw an error if trying to insert an sectionCN with instittue that do not exists', function* () {
            let error;
            try {
                yield sectionCNQueries.insertOne({ name: 'section', code: '53', primary_institutes: [primary.id, 'nemo'] });
            } catch (e) {
                error = e;
            }
            assert.equal(error.message, 'Institutes nemo does not exists');

            const insertedSectionCN = yield postgres.queryOne({sql: 'SELECT * from section_cn WHERE name=$name', parameters: { name: 'section'} });
    ...
    describe('selectByIds', function () {
        ...
        before(function*  () {
            [section1, section2] = yield ['1', '2', '3']
            .map(code => fixtureLoader.createSectionCN({ code, name: `SectionCN ${code}` }));
        });

        it('should return each sectionsCN with given ids', function* () {

            assert.deepEqual(yield sectionCNQueries.selectByIds([section1.id, section2.id]), [
                {
                    id: section1.id,
                    name: section1.name,
                    code: section1.code
