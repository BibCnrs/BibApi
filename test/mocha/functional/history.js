...
describe('/ebsco/history', function () {
    ...
    before(function* () {
        ...
        historyEntries = yield Array.from(Array(12).keys()).map((key) => fixtureLoader.createHistory({ user_id: janusAccount.id, created_at: subMinutes(new Date, key) }));
        historyEntries = historyEntries.sort((h1, h2) => h2.created_at - h1.created_at);
    });
    ...
    describe('GET', function () {
        it('should return first page of user history entries', function* () {
            ...
            const expected = historyEntries.slice(0, 5).map(({ id, event }) => ({
                id,
                event,
                totalcount: '12',
            }));
        ...
        it('should return requested page of user history entries', function* () {
            ...
            assert.deepEqual(historyEntriesFromServer, historyEntries.slice(10, 13).map(({ id, event }) => ({
                id,
                event,
                totalcount: '12',
            })));
    ...
    describe('POST', function () {
        it('should save the history entry', function* () {
            request.setToken({ id: janusAccount.id, username: 'john' });
            yield request.post('/ebsco/history', {
                history: { bar: 'foo' },
            }, null);
