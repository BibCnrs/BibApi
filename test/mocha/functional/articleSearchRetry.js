...
describe('Retry GET /ebsco/:domainName/article/search', function () {
    ...
    before(function* () {
        ...
        yield fixtureLoader.createJanusAccount({ uid: 'vie', communities: [vie.id] });
    ...
    beforeEach(function* () {
        ...
        apiServer.router.post('/edsapi/rest/Search', function* (next) {
            ...
            this.status = 400;
            this.body = {
                DetailedErrorDescription: 'Invalid Session Token. Please generate a new one.',
    ...
    it('should return search result directly if session token is good', function* () {
        ...
        assert.deepEqual(ebscoCall, [{ name: 'search', authToken: 'auth-token-for-vie', sessionToken: 'session-token-for-vie-0' }]);
    ...
    it('should retry with new sessionToken until the token get accepted(4times) ', function* () {
        ...
        assert.deepEqual(ebscoCall, [
            { name: 'search', authToken: 'auth-token-for-vie', sessionToken: 'session-token-for-vie-0' },
            { name: 'session', Guest: 'n', Profile: 'profileVie', authToken: 'auth-token-for-vie' },
            { name: 'search', authToken: 'auth-token-for-vie', sessionToken: 'session-token-for-vie-1' },
            { name: 'session', Guest: 'n', Profile: 'profileVie', authToken: 'auth-token-for-vie' },
            { name: 'search', authToken: 'auth-token-for-vie', sessionToken: 'session-token-for-vie-2' },
            { name: 'session', Guest: 'n', Profile: 'profileVie', authToken: 'auth-token-for-vie' },
            { name: 'search', authToken: 'auth-token-for-vie', sessionToken: 'session-token-for-vie-3' }
        ]);
        assert.deepEqual(JSON.parse(response.body), aidsResult);
    });
